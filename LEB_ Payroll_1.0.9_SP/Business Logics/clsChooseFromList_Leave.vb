...
#Region "Bind Data"
    ...
    Public Sub databound(ByVal objform As SAPbouiCOM.Form)
        Try
            ...
            oRecS.DoQuery("Select isnull(U_Z_Terms,'') from OHEM where empID=" & ItemCode)
            ...
            If choice = "MODULE" Then
                Select Case CFLChoice
                    Case "L"
                        '  strSQL = "Select ""U_Z_LeaveCode"" 'OverTime Code',""U_Z_OVTCODE"" 'Overtime Name',""U_Z_OVTRATE"" 'Over Time Rate' from [@Z_PAY_OOVT]"
                        oRecS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                        oRecS.DoQuery("Select isnull(U_Z_Terms,'') from OHEM where empID=" & ItemCode)
                        If oRecS.Fields.Item(0).Value = "" Then
                            strSQL = "Select Code,Name  from  [@Z_PAY_LEAVE]  "
                        ...
                        End If
                        objform.Title = "Leave Code - Selection"
                    Case "E"
                        ...
                        If pVal.EventType = SAPbouiCOM.BoEventTypes.et_DOUBLE_CLICK And pVal.Row <> -1 Then
                            ...
                            If CFLChoice <> "" Then
                                ...
                                If choice <> "Ticket" Then
                                    ...
                                    objGrid.DataTable.SetValue("U_Z_LeaveName", sourcerowId, strSelectedItem2)
                                    If oForm.TypeEx = frm_PayLeaveTrans Then
                                        Dim oObj As New clsPayrollLeaveTransaction
                                        oObj.populateDetails(objGrid, sourcerowId, oForm)
                                    End If

                                    If oForm.TypeEx = frm_LeaveEncashement Then
                                        Dim oObj As New clsLeaveEncashement
                                        oObj.populateDetails(objGrid, sourcerowId, oForm)
                                    End If
                                    'If CFLChoice = "L" Or CFLChoice = "D" Then
